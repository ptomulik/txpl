...
#define BOOST_TEST_MODULE test_txpl_parser_parse_impl_expr8

#include <txpl/test_config.hpp>
#include <boost/test/unit_test.hpp>

#ifndef TXPL_TEST_SKIP_PARSER_PARSE_IMPL_EXPR8
...
BOOST_AUTO_TEST_CASE(test__parse__expr_8__literal__op__literal)
{
  ...
  ast::expr<token_iterator, vm::basic_types<>, 8> e;
  ...
  ast::literal<token_iterator> l2;
  {
    ...
    lexer::token<> tok[] = {
      ...
      lexer::token<>(lexer::token_t::lt_, beg[1], end[1]),
    ...
    BOOST_CHECK(e.tail.front().operator_ == vm::op_t::lt_);
  ...
  }
  {
    ...
    lexer::token<> tok[] = {
      lexer::token<>(lexer::token_t::int_, beg[0], end[0]),
      lexer::token<>(lexer::token_t::gt_, beg[1], end[1]),
      lexer::token<>(lexer::token_t::int_, beg[2], end[2])
    };
    lexer::token<> const* first = tok, *last = first + 3;
    ...
    BOOST_CHECK_NO_THROW(x1 = get<int_type>(l1.value));
    BOOST_CHECK_NO_THROW(x2 = get<int_type>(l2.value));
    BOOST_CHECK(e.tail.front().operator_ == vm::op_t::gt_);
    BOOST_CHECK_EQUAL(x1, int_type{456});
    BOOST_CHECK_EQUAL(x2, int_type{123});
    BOOST_CHECK(first == last);
  }
  {
    ...
    lexer::token<> tok[] = {
      lexer::token<>(lexer::token_t::int_, beg[0], end[0]),
      lexer::token<>(lexer::token_t::le_, beg[1], end[1]),
      lexer::token<>(lexer::token_t::int_, beg[2], end[2])
    };
    lexer::token<> const* first = tok, *last = first + 3;
    ...
    BOOST_CHECK_NO_THROW(x1 = get<int_type>(l1.value));
    BOOST_CHECK_NO_THROW(x2 = get<int_type>(l2.value));
    BOOST_CHECK(e.tail.front().operator_ == vm::op_t::le_);
    BOOST_CHECK_EQUAL(x1, int_type{456});
    BOOST_CHECK_EQUAL(x2, int_type{123});
    BOOST_CHECK(first == last);
  }
  {
    ...
    lexer::token<> tok[] = {
      lexer::token<>(lexer::token_t::int_, beg[0], end[0]),
      lexer::token<>(lexer::token_t::ge_, beg[1], end[1]),
      lexer::token<>(lexer::token_t::int_, beg[2], end[2])
    };
    lexer::token<> const* first = tok, *last = first + 3;
    ...
    BOOST_CHECK_NO_THROW(x1 = get<int_type>(l1.value));
    BOOST_CHECK_NO_THROW(x2 = get<int_type>(l2.value));
    BOOST_CHECK(e.tail.front().operator_ == vm::op_t::ge_);
    BOOST_CHECK_EQUAL(x1, int_type{456});
    BOOST_CHECK_EQUAL(x2, int_type{123});
    BOOST_CHECK(first == last);
  }
