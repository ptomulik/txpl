...
#define BOOST_TEST_MODULE test_txpl_parser_parse_impl_expr6

#include <txpl/test_config.hpp>
#include <boost/test/unit_test.hpp>

#ifndef TXPL_TEST_SKIP_PARSER_PARSE_IMPL_EXPR6
...
BOOST_AUTO_TEST_CASE(test__parse__expr_6__literal__op__literal)
{
  ...
  ast::expr<token_iterator, vm::basic_types<>, 6> e;
  ...
  ast::literal<token_iterator> l2;
  {
    ...
    lexer::token<> tok[] = {
      ...
      lexer::token<>(lexer::token_t::plus_, beg[1], end[1]),
  ...
  }
  {
    ...
    lexer::token<> tok[] = {
      lexer::token<>(lexer::token_t::int_, beg[0], end[0]),
      lexer::token<>(lexer::token_t::minus_, beg[1], end[1]),
      lexer::token<>(lexer::token_t::int_, beg[2], end[2])
    };
    lexer::token<> const* first = tok, *last = first + 3;
