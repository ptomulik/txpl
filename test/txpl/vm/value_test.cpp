...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  {
    BOOST_CHECK_NO_THROW(boost::get<blank_type>(v));
    BOOST_CHECK_THROW(boost::get<char_type>(v), boost::bad_get);
    BOOST_CHECK_THROW(boost::get<int_type>(v), boost::bad_get);
    BOOST_CHECK_THROW(boost::get<bool_type>(v), boost::bad_get);
    BOOST_CHECK_THROW(boost::get<real_type>(v), boost::bad_get);
    BOOST_CHECK_THROW(boost::get<string_type>(v), boost::bad_get);
    BOOST_CHECK_THROW(boost::get<regex_type>(v), boost::bad_get);
  }
  {
    char_type x;
    BOOST_CHECK_NO_THROW(v = char_type{'a'});
    BOOST_CHECK_NO_THROW(x = boost::get<char_type>(v));
    BOOST_CHECK_EQUAL(x, char_type{'a'});
  }
  {
    int_type x;
    BOOST_CHECK_NO_THROW(v = int_type{100});
    BOOST_CHECK_NO_THROW(x = boost::get<int_type>(v));
    BOOST_CHECK_EQUAL(x, int_type{100});
  }
  {
    bool_type x;
    BOOST_CHECK_NO_THROW(v = bool_type{true});
    BOOST_CHECK_NO_THROW(x = boost::get<bool_type>(v));
    BOOST_CHECK_EQUAL(x, bool_type{true});
  }
  {
    real_type x;
    BOOST_CHECK_NO_THROW(v = real_type{.100});
    BOOST_CHECK_NO_THROW(x = boost::get<real_type>(v));
    BOOST_CHECK_EQUAL(x, real_type{.100});
  }
  {
    string_type x;
    BOOST_CHECK_NO_THROW(v = string_type("xyz"));
    BOOST_CHECK_NO_THROW(x = boost::get<string_type>(v));
    BOOST_CHECK_EQUAL(x, string_type("xyz"));
  }
  {
    BOOST_CHECK_NO_THROW(v = regex_type());
    BOOST_CHECK_NO_THROW(boost::get<regex_type>(v));
  }
  {
    array_type x, y;
    char_type c = 'x';
    ...
    BOOST_CHECK_NO_THROW(x = boost::get<array_type>(v));
    BOOST_CHECK_EQUAL(x.size(), 2);
    BOOST_CHECK_NO_THROW(c = boost::get<char_type>(x[0]));
    BOOST_CHECK_NO_THROW(i = boost::get<int_type>(x[1]));
  ...
  }
  {
    ...
    y["i"] = int_type{100};
    BOOST_CHECK_NO_THROW(v = y);
    BOOST_CHECK_NO_THROW(x = boost::get<object_type>(v));
    BOOST_CHECK_EQUAL(x.size(), 2);
    BOOST_CHECK_NO_THROW(c = boost::get<char_type>(x.at("c")) );
    BOOST_CHECK_NO_THROW(i = boost::get<int_type>(x.at("i")) );
    BOOST_CHECK(c == 'a');
    BOOST_CHECK(i == 100);
  }
}
