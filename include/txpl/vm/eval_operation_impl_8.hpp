...
struct eval_operation_impl<8ul>
{
  ...
  apply(op_t op, Lhs& lhs, Rhs const& rhs, Ehandler f)
  {
    ...
    {
      case op_t::eq_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::eq_>(lhs), lhs, rhs))
          {
        ...
        break;
      case op_t::neq_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::neq_>(lhs), lhs, rhs))
          {
            f("invalid operands to binary operator '!='");
            return false;
          }
        break;
      case op_t::match_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::match_>(lhs), lhs, rhs))
          {
            f("invalid operands to binary operator '=~'");
            return false;
          }
        break;
      case op_t::nmatch_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::nmatch_>(lhs), lhs, rhs))
          {
            f("invalid operands to binary operator '!~'");
            return false;
          }
