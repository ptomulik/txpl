 ...
 */ // }}}
#ifndef TXPL_VM_EVAL_OPERATION_IMPL_5_HPP_INCLUDED
#define TXPL_VM_EVAL_OPERATION_IMPL_5_HPP_INCLUDED
...
/** // doc: eval_operation_impl<5ul> {{{
 * \todo Write documentation
 */ // }}}
template<>
struct eval_operation_impl<5ul>
{
  /** // doc: apply() {{{
   * \todo Write documentation
   */ // }}}
  ...
  apply(op_t op, Lhs& lhs, Rhs const& rhs, Ehandler f)
  {
    ...
    {
      case op_t::plus_:
        if(!boost::apply_visitor(eval_binary_op<op_t::plus_>(lhs), lhs, rhs))
          {
        ...
        break;
      case op_t::minus_:
        if(!boost::apply_visitor(eval_binary_op<op_t::minus_>(lhs), lhs, rhs))
          {
            f("invalid operands to binary operator '-'");
            return false;
          }
        break;
      default:
        f("invalid operator for level-5 operation");
        return false;
    }
    return true;
  }
...
#endif /* TXPL_VM_EVAL_OPERATION_IMPL_5_HPP_INCLUDED */
