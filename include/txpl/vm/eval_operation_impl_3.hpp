...
struct eval_operation_impl<3ul>
{
  ...
  apply(op_t op, Lhs& lhs, Rhs const& rhs, Ehandler f)
  {
    ...
    {
      case op_t::mul_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::mul_>(lhs), lhs, rhs))
          {
        ...
        break;
      case op_t::div_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::div_>(lhs), lhs, rhs))
          {
            f("invalid operands to binary operator '/'");
            return false;
          }
        break;
      case op_t::mod_:
        if(!yaul::apply_visitor(eval_binary_op<op_t::mod_>(lhs), lhs, rhs))
          {
            f("invalid operands to binary operator '%'");
            return false;
          }
