...
struct eval_impl<ast::unary_expr<Iterator, BasicTypes> >
{
  /** // doc: node_type {{{
   * \todo Write documentation
   */ // }}}
  ...
  {
    Result subresult;
    if(!eval(node.expr, context, subresult, ehandler)) return false;
    switch(node.operator_)
      {
        case op_t::plus_:
          if(!boost::apply_visitor(eval_unary_op<op_t::plus_, BasicTypes>(result), subresult))
            {
              // FIXME: invoke error handler:
              // ehandler(???, ???, "wrong type argument to unary operator '+'")
              return false;
            }
          break;
        case op_t::minus_:
          if(!boost::apply_visitor(eval_unary_op<op_t::minus_, BasicTypes>(result), subresult))
            {
              // FIXME: invoke error handler:
              // ehandler(???, ???, "wrong type argument to unary operator '-'")
              return false;
          ...
          break;
        case op_t::not_:
          if(!boost::apply_visitor(eval_unary_op<op_t::not_, BasicTypes>(result), subresult))
            {
              // FIXME: invoke error handler:
              // ehandler(???, ???, "wrong type argument to unary operator '!'")
              return false;
          ...
          break;
        case op_t::neg_:
          if(!boost::apply_visitor(eval_unary_op<op_t::neg_, BasicTypes>(result), subresult))
            {
              // FIXME: invoke error handler:
              // ehandler(???, ???, "wrong type argument to unary operator '~'")
              return false;
