...
// txpl/lexer/try_hex_number.hpp

/** // doc: txpl/lexer/try_hex_number.hpp {{{
 * \file txpl/lexer/try_hex_number.hpp
 * \brief Implements \ref txpl::lexer::try_hex_number() "lexer::try_hex_number()"
 */ // }}}
#ifndef TXPL_LEXER_TRY_HEX_NUMBER_HPP_INCLUDED
#define TXPL_LEXER_TRY_HEX_NUMBER_HPP_INCLUDED
...
/** // doc: try_hex_number() {{{
 * \ingroup LexerGroup
 * \brief Scan for a hexadecimal number
 *
 ...
 * "hex-number" without semantic value retrieval.
 *
 * This function scans the content between iterators \e first and \e last
 * trying to recognize a hexadecimal number. On success, the iterator \e first
 * is moved to the next position after the last scanned input character.
...
bool try_hex_number(Iterator& first, Iterator const& last)
{
  if(!try_hex_digit(first, last)) return false;
  while(try_hex_digit(first, last)) {}
  return true;
}
} } // end namespace txpl::lexer

#endif /* TXPL_LEXER_TRY_HEX_NUMBER_HPP_INCLUDED */
